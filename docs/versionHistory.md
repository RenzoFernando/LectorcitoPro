...
## Versión 10: Multiplataforma, Markdown y Rediseño Visual

1. Implementado un sistema global de escalado, DPI y métricas visuales adaptativas para mantener proporciones correctas en distintas resoluciones, densidades de pantalla y configuraciones de escala.

2. Reconstruidos el encabezado y el menú principal con comportamiento adaptativo, mejorando el ajuste del saludo, la tipografía, la distribución vertical y la armonía entre la ventana principal y los submenús.

3. Refactorizada la arquitectura multiplataforma mediante una capa independiente de servicios del sistema operativo, aislando las integraciones específicas de Windows y preparando un núcleo común reutilizable.

4. Añadida compatibilidad funcional con Linux y una distribución portable Onefile x86_64, incorporando rutas XDG, apertura nativa de archivos y carpetas, integración de escritorio y comportamiento equivalente donde aplica.

5. Convertido Markdown en un formato de reporte real y profesional, con bloques de código por lenguaje, fences robustos, navegación interna, tabla de contenidos, anchors estables y nombres de archivos localizados por idioma.

6. Separados el escaneo, el modelo neutral del reporte, el procesamiento, los renderizadores TXT/Markdown y la internacionalización, eliminando dependencias impropias entre Model y View y facilitando futuras extensiones.

7. Reorganizada la infraestructura de build, firma y release en scripts especializados para Windows y Linux, con un punto único de entrada, generación de portable e instalable Windows, portable Linux, validaciones, logs y artefactos centralizados.

8. Profesionalizadas las dependencias y el entorno de ejecución, separando runtime, Windows, Linux, build y desarrollo; además se realizó el pulido técnico de rendimiento, logging, encoding, manejo de errores, side effects, apertura externa, detección de runtime y delays visuales.

9. Renovada la identidad visual de controles y barras laterales, armonizando colores, estados, botones, iconografía y contraste para ofrecer una presentación más limpia y coherente en los temas claro y oscuro.

10. Refinada la distribución general de la interfaz y los componentes de configuración, mejorando las burbujas y etiquetas de Qué Ver/Qué No Ver, los espacios internos, la organización de paneles y la consistencia visual global de la aplicación.
