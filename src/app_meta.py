...
APP_REPOSITORY_WEB_URL = (
    APP_REPOSITORY_URL[:-4] if APP_REPOSITORY_URL.endswith(".git") else APP_REPOSITORY_URL
)
...
def build_web_meta_js() -> str:
    ...
    return (
        "window.APP_META = "
        + json.dumps(get_web_meta_payload(), ensure_ascii=False, indent=4)
        + ";\n"
    )
