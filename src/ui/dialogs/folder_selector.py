...
class SelectFoldersDialog(BaseDialog):
    def __init__(self, parent, title):
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(
            list_area_frame, label_text=self._parent._tr("dlg_multi_list_title")
        )
    ...
    def _update_folder_list(self):
        ...
        if not self.selected_paths:
            ...
            for i, path in enumerate(self.selected_paths):
                ...
                label = ctk.CTkLabel(
                    item_frame, text=f"{i + 1}. {path}", anchor="w", compound="left", padx=10, font=("Segoe UI", 10)
                )
                label.pack(fill="x")
    ...
    def _move_up(self):
        if 0 < self.currently_selected_index < len(self.selected_paths):
            ...
            self.selected_paths[idx], self.selected_paths[idx - 1] = (
                self.selected_paths[idx - 1],
                self.selected_paths[idx],
            )
            self.currently_selected_index -= 1
            self._update_folder_list()
