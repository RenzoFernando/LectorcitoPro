...
def file_rules_conflict(rule_a: str, rule_b: str) -> bool:
    ...
    return normalized_a == normalized_b or canonical_file_rule(normalized_a) == canonical_file_rule(
        normalized_b
    )
...
def normalize_file_tag_list(tag_list: list | None) -> list[dict]:
    ...
    for tag in tag_list or []:
        ...
        if not normalized_name:
            continue
        normalized_tags.append({"nombre": normalized_name, "estado": tag.get("estado", "activo")})
